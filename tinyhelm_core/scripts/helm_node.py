...
from std_msgs.msg import String, Bool, Empty, ColorRGBA, Float32
...
class HelmCore:
	def __init__(self):
		...
		self.speed_scale_topic = cmd_mux_cfg.get("speed_scale_topic", "/cmd_vel_mux/_speed_scale")
		self.slow_speed_scale = cmd_mux_cfg.get("slow_speed_scale", 0.5)

		self.tf2_buffer = tf2_ros.Buffer()
		self.tf2_listener = tf2_ros.TransformListener(self.tf2_buffer)
		self.selector_pub = rospy.Publisher(self.selector_topic, String, queue_size=1)
		self.speed_scale_pub = rospy.Publisher(self.speed_scale_topic, Float32, queue_size=1, latch=True)
	...
	def refresh_speed_scale(self):
		...
		self.speed_scale_pub.publish(Float32(scale))
